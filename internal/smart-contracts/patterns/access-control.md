...
**Layer 2: Document Ownership Model**
...
Layer 3 implements opt-in executor authorization with zero-trust defaults:
...
```solidity
/**
 * Token claim requires ALL THREE LAYERS to pass:
 *
 * Layer 0: Caller must have CORE_CLAIM capability (attestation proof)
 * Layer 2: Document must exist and have valid owner
 * Layer 3: Caller must be owner OR authorized executor
 */
function claimToken(
    bytes32 integraHash,
    uint256 tokenId,
    ...
    // Layer 0: Verify capability via attestation
    requiresCapability(integraHash, CAPABILITY_CLAIM_TOKEN, attestationProof)
    // Layer 3: Verify owner or executor
    requireOwnerOrExecutor(integraHash)
    // Security modifiers
    nonReentrant
    whenNotPaused
{
    // Layer 2: Implicit in requireOwnerOrExecutor (checks document exists)
...
1. **Layer 0 Compromise**: Even if attestation system compromised, still need Layer 2/3 authorization
2. **Layer 2 Compromise**: Even if owner key compromised, attestations limit damage scope
3. **Layer 3 Compromise**: Even if executor compromised, owner can immediately revoke
...
- [Layer 0 Documentation](../layer0/) - Attestation system details
