...
AttestationAccessControl is the abstract base contract for provider-agnostic attestation access control with progressive ossification. It provides the core capability verification infrastructure that all Layer 1+ contracts inherit.
...
- Immutable references to CapabilityNamespace and AttestationProviderRegistry
...
AttestationProviderRegistry_Immutable public PROVIDER_REGISTRY;
```
...
- **CapabilityNamespace**: Referenced for capability definitions
...
```solidity
contract IntegraDocumentRegistry is AttestationAccessControl {
...
```solidity
contract MyDocumentRegistry is AttestationAccessControl {
    mapping(bytes32 => address) public documentOwners;
...
```solidity
contract FlexibleDocumentSystem is AttestationAccessControl {
