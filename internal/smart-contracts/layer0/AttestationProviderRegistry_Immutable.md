...
AttestationProviderRegistry_Immutable is an immutable registry for attestation providers (EAS, VC, ZK, DIDs) with code integrity verification. It prevents malicious provider upgrades through code hash validation.
...
- **AttestationAccessControl**: Uses getProvider() to lookup providers
- **EASAttestationProvider**: Registered as a provider
...
```solidity
// Deploy registry
AttestationProviderRegistry_Immutable registry =
    new AttestationProviderRegistry_Immutable(governorAddress);
...
```solidity
import "./AttestationProviderRegistry_Immutable.sol";

contract MyAccessControl {
    AttestationProviderRegistry_Immutable public immutable PROVIDER_REGISTRY;
...
```javascript
const Registry = await ethers.getContractFactory("AttestationProviderRegistry_Immutable");
