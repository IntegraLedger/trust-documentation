...
**License**: MIT
**Inherits**: AttestationAccessControl, IDocumentTokenizer

BaseTokenizer is the abstract base contract for all document tokenizers. It provides common functionality including access control, validation helpers, and integration with the document registry. All concrete tokenizers inherit from this contract.
...
- Updated to AttestationAccessControl (provider abstraction)
...
BaseTokenizer is abstract because:
...
- **Inherits From**: AttestationAccessControl (Layer 0)
- **References**: IntegraDocumentRegistry_Immutable (Layer 2)
- **Implements**: IDocumentTokenizer (interface)
...
BaseTokenizer implements a zero-trust model with exactly three access paths (in priority order):
...
function __BaseTokenizer_init(
    ...
    // Initialize parent (AttestationAccessControl)
...
```solidity
contract OwnershipTokenizer is BaseTokenizer, ERC721Upgradeable {
...
```solidity
// Maps to CapabilityNamespace.CORE_CLAIM (1 << 1)
...
Abstract method implemented by inherited AttestationAccessControl contract:
...
```solidity
// 1. Deploy tokenizer
OwnershipTokenizer tokenizer = new OwnershipTokenizer();
...
OwnershipTokenizer(address(proxy)).initialize(
    "Property Deeds",
    "DEED",
    "ipfs://base-uri/",
    governor,
...
// 5. Reserve token
OwnershipTokenizer(address(proxy)).reserveToken(
    integraHash,
    0,
    buyerAddress,
    1,
...
**Path**: `/src/layer3/base/BaseTokenizer.sol`
**Version**: 7.0.0
**Audited**: Yes (Layer 0-3 Security Audit 2024)

## Related Contracts

- [AttestationAccessControl](../layer0/AttestationAccessControl.md) - Parent contract
- [IntegraDocumentRegistry_Immutable](../layer2/IntegraDocumentRegistry_Immutable.md) - Referenced registry
- [TrustGraphIntegration](./TrustGraphIntegration.md) - Trust credential mixin
- [IDocumentTokenizer](./IDocumentTokenizer.md) - Interface
- [OwnershipTokenizer](./OwnershipTokenizer.md) - Example concrete implementation
