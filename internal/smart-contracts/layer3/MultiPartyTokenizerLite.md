...
**Inherits**: BaseTokenizer, TrustGraphIntegration

MultiPartyTokenizerLite implements gas-optimized multi-stakeholder document tokenization using ERC-6909, a lightweight alternative to ERC-1155. It provides 50% gas savings over standard ERC-1155 by eliminating mandatory callbacks and simplifying the approval system.

### Purpose

- Tokenize multi-party documents with maximum gas efficiency
- Enable high-volume multi-stakeholder scenarios
- Provide lightweight alternative to MultiPartyTokenizer
...
To save gas, MultiPartyTokenizerLite doesn't track holders:

```solidity
// MultiPartyTokenizer (full version):
address[] holders;
...
Same interface as MultiPartyTokenizer:
...
// Standard IDocumentTokenizer event
emit TokenClaimed(...);
    ...
    external view override returns (IDocumentTokenizer.TokenInfo memory)
{
    ...
    return IDocumentTokenizer.TokenInfo({
...
{
    return IDocumentTokenizer.TokenType.CUSTOM;  // ERC-6909
}
```
...
```solidity
// 1. DEPLOY
MultiPartyTokenizerLite multiPartyLite = new MultiPartyTokenizerLite();
...
```solidity
// Same as MultiPartyTokenizer, but cheaper gas
...
### Related Tokenizers
- **MultiPartyTokenizer**: Full-featured ERC-1155 version (use when gas less important)
- **SemiFungibleTokenizer**: Similar use case, different focus

## Comparison

### MultiPartyTokenizerLite vs MultiPartyTokenizer
...
- [MultiPartyTokenizer](./MultiPartyTokenizer.md) - Full-featured version
