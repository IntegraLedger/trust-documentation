...
VaultTokenizer implements custody and escrow document tokenization using ERC-721 NFTs. Each token represents unique custody rights or vault access, with optional value tracking for collateral.
...
VaultTokenizer doesn't include TrustGraphIntegration:
...
```solidity
// 1. DEPLOY
VaultTokenizer vaultTokenizer = new VaultTokenizer();
...
### Related Tokenizers
- **OwnershipTokenizer**: Similar ERC-721 for general ownership (not custody-specific)
- **RentalTokenizer**: Use for time-based access (not permanent custody)
...
**Q: What's the difference between VaultTokenizer and OwnershipTokenizer?**
...
A: No, use OwnershipTokenizer. Vault is for custody/escrow scenarios.

**Q: Can multiple people have custody?**
A: No (ERC-721 = single owner). For multi-party custody, use MultiPartyTokenizer.

**Q: How do I implement escrow conditions?**
A: Off-chain or separate smart contract. VaultTokenizer tracks custody, not conditions.
...
- [OwnershipTokenizer](./OwnershipTokenizer.md) - Similar ERC-721 tokenizer
