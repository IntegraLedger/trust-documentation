...
The tokenization layer provides tokenization strategies for different document types in the Integra V7 system. Each tokenizer implements the token lifecycle (reserve → claim) with attestation-based access control and optional trust graph integration.

### Purpose

- Transform registered documents (from document contracts) into blockchain tokens
...
```
Document Registry → Tokenization Contracts → Tokens
         ↓                      ↓                ↓
...
- [Document Management](../layer2/overview.md) - Document registry
- [Foundation](../layer0/overview.md) - Access control foundation
