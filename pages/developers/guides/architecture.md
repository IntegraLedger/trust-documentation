# Integra V7 Architecture for Developers

Understanding how Integra's smart contracts work together to enable document tokenization and management.

## Overview

Integra V7 provides a layered architecture for tokenizing real-world documents on the blockchain. As a developer, you'll interact with contracts across different layers depending on your use case.

## Contract Layers

### Foundation Layer: Core Registries

These immutable contracts provide the security and capability framework. You'll reference these when verifying permissions and looking up providers.

**Key Contracts:**
- **CapabilityNamespaceV7_Immutable** - Defines permission capabilities (CORE_CLAIM, CORE_TRANSFER, etc.)
- **AttestationProviderRegistryV7_Immutable** - Registry of attestation providers (like EAS)
- **IntegraVerifierRegistryV7_Immutable** - Registry of ZK proof verifiers
- **IntegraResolverRegistryV7_Immutable** - Registry of document resolvers

**When you'll use them:**
- Checking if a user has specific capabilities
- Looking up which attestation provider to use
...
// Now users can look up contact info
address contactResolver = resolverRegistry.getResolver(CONTACT_RESOLVER_ID);
