...
IntegraDocumentRegistry.registerDocument(
...
<div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0' }}>
  <img src="/diagrams/contract-work-together.png" alt="How Contracts Work Together" style={{ width: '90%', height: 'auto' }} />
</div>
...
RentalTokenizer rentalTokenizer = RentalTokenizer(tokenizerAddress);
...
RoyaltyTokenizer royaltyTokenizer = RoyaltyTokenizer(tokenizerAddress);
...
MultiPartyTokenizer multiParty = MultiPartyTokenizer(tokenizerAddress);
...
**See**: [TokenClaimResolver Documentation](./document-registration/token-claim-resolver.md)
...
<Tabs items={['curl', 'TypeScript', 'Python']}>
  <Tabs.Tab>
    ...
    curl https://integra-chain-registry-worker.dfisher-3f3.workers.dev/v1/chains/polygon/contracts
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```typescript
  ...
  <Tabs.Tab>
    ...
    # Or using the SDK (if available)
    # from integra_sdk import get_contract_addresses
    # addresses = get_contract_addresses('polygon')
    ```
  </Tabs.Tab>
</Tabs>
